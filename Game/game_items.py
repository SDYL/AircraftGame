...
class GameSprite(pygame.sprite.Sprite):
    """游戏精灵类"""
    res_path = "../Game/images//"       # 图片资源路径

    def __init__(self, image_name, speed, *groups):
        """初始化方法
        :param image_name:要加载的图片
...
class Label(pygame.sprite.Sprite):
    """文本标签类"""
    font_path = "../Game/font/MarkerFelt.ttc"          # 字体文件路径
    ...
    def set_text(self, text):
        """设置文本，使用指定的文本重新渲染 image ，并且更新rect
        :param text: 文本内容
        """
        self.image = self.font.render(text, True, self.color)
        self.rect = self.image.get_rect()
...
class Enemy(Plane):
    ...
    def __init__(self, kind, max_speed, *groups):
        ...
        if kind == 0:
            ...
            super().__init__(6, 1, 1000, "enemy2_down.wav",
                             ["enemy2.png"],
                             "enemy2.png",
            ...
            super().__init__(15, 1, 1500, "enemy3_down.wav",
                             ["enemy3.png"],
                             "enemy3.png",
    ...
    def update(self, *args):
        """更新图像和位置"""
        # 调用父类方法跟新飞机图像 - 注意args要拆包
        super().update(*args)
        # 判断敌机是否已经被摧毁，否则使用速度更新飞机位置
        if self.hp > 0:
            self.rect.y += self.speed
...
class Hero(Plane):
    ...
    def __init__(self, *groups):
        ...
        self.bomb_count = HERO_BOMB_COUNT             # 炸弹标记
    ...
    def update(self, *args):
        """更新英雄的图像以及矩形区域
        :param args:  0 更新图像编辑 1 次 水平移动基数 2 垂直移动基数
        """
